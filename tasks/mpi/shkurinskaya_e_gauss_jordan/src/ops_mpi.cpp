...
bool shkurinskaya_e_gauss_jordan_mpi::TestMPITaskParallel::run() {
  ...
  for (int k = 0; k < n; ++k) {
    if (world.rank() == 0) {
      ...
      send—ounts = std::vector<int>(world.size(), elements_per_process);
      for (int i = 0; i < remainder / (n + 1); i++) {
        send—ounts[i] += (n + 1);
      }
      displacements = std::vector<int>(world.size(), offset);
      for (int i = 1; i < world.size(); ++i) {
        displacements[i] = displacements[i - 1] + send—ounts[i - 1];
      }
    }
    boost::mpi::broadcast(world, header, 0);
    boost::mpi::broadcast(world, send—ounts, 0);
    boost::mpi::broadcast(world, displacements, 0);

    localMatrix.resize(send—ounts[world.rank()]);
    boost::mpi::scatterv(world, matrix, send—ounts, displacements, localMatrix.data(), send—ounts[world.rank()], 0);
    ...
    boost::mpi::gatherv(world, localMatrix, matrix.data(), send—ounts, displacements,
                        0);
  }
  for (int k = n - 1; k >= 0; --k) {
    if (world.rank() == 0) {
    ...
    boost::mpi::gatherv(world, localMatrix, matrix.data(), send—ounts, displacements, 0);
  }
  return true;
}
